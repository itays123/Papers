/**
 * Added process_status() function by Itay Schechner 328197462
 */
#include "types.h"
#include "defs.h"
#include "param.h"
...
int growproc(int n)
{
    ...
    sz = curproc->sz;
    if (n > 0)
    { // In this case we update protected memory inside of allocuvm function
...
int process_status()
{
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        ...
        // Retrieve ppid and extpid fields.
        ppid = p->parent->ns_pid; // default case, change if init process
        extpid = p->pids[1].pid;  // default case, change if not in container

        if (extpid == 0)
        { // not in container, might be INIT process
            extpid = p->pids[0].pid;
            if (extpid == 1) // the init process
                ppid = 0;
        }
        ...
        // print row
        cprintf("%s \t %d \t %s \t %d \t \t %d \t %d \n ", p->name, p->ns_pid, status, extpid, ppid, p->cpu_time);
    }
    release(&ptable.lock);

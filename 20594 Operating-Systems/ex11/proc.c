/**
 * Functions related to proccesses.
 * Modified by Itay Schechner 328197462
 * Added process_status function.
 */
#include "types.h"
#include "defs.h"
#include "param.h"
...
int process_status(void)
{
    ...
    char *state;
    int pid, ppid;
    cprintf("name \t pid \t status \t\t ppid \n");
    // iterate through process table, print information about each process.
    acquire(&ptable.lock);
    for (currentProc = ptable.proc; currentProc < &ptable.proc[NPROC]; currentProc++)
    {
        ...
        cprintf("%s \t %d \t %s \t\t %d \n", currentProc->name, pid, state, ppid);
    }
    release(&ptable.lock);
    return 0;
}